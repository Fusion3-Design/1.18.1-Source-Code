...
void UTFT::setXY(uint16_t p_x1, uint16_t p_y1, uint16_t p_x2, uint16_t p_y2)
{
	...
	switch(displayModel)
	{
		...
		default:
			//x's are the rows
			x1 = x1 + frameBufferAddress;
			x2 = x2 + frameBufferAddress;
...
{
	uint16_t r=fcolour>>11;
	uint16_t g=(fcolour>>5) & 0x3F;
	uint16_t b=(fcolour) & 0x1F;

	int8_t gr=((grad>>8) & 0xF8)/8;
	int8_t gg=((grad>>3) & 0xFC)/4;
	int8_t gb=((grad<<3) & 0xF8)/8;
	...
	//fcolour += grad;
	fcolour=(r & 0x1F)<<11 | (g & 0x3f)<<5 | (b & 0x1F);
}

Colour UTFT::applyColorScale(uint8_t percent, Colour c)
{
	Colour x;
	uint16_t r=c>>11;
	uint16_t g=(c>>5) & 0x3F;
...
void UTFT::fillRect(int x1, int y1, int x2, int y2, Colour grad, uint8_t gradChange,bool vertical)
{
	if (x1>x2)
	{
		swap(x1, x2);
	...
	uint8_t gradCount = 0;
	if (orient & SwapXY && vertical==false)
	{
		for (int i = x1; i <= x2; i++)
		{
...
void UTFT::drawBinaryBitMap(int x, int y, int sx, int sy, const uint8_t * data)
{
	int curY = y;
	uint16_t scale=1;
	uint8_t bit=0x01;
	...
	for (int ty = 0; ty < sy; ty++)
	{
		...
		for (int i = 0; i < scale; ++i)
		{
			...
			uint16_t idx=(ty* (sx & 0x03)?(sx/8+1):(sx/8)) ;  //account for bytes if not multiple of 8
			...
			for (int tx = 0; tx < sx; tx++)
			{
				...
				if (transparentBackground && color == 0xFFFF)
				{
					xySet = false;
				}
				else
				{
					...
					LCD_Write_Repeated_DATA16(color, scale);
				}
			}
			++curY;
		}
...
bool UTFT::SetWriteFrameBuffer(uint8_t i)
{
	WriteFrameBufferIndex=i;
	frameBufferAddress = (disp_y_size + 1) * i;
	LOG("Set Line to %d", frameBufferAddress);
}

bool UTFT::ShowFrameBuffer(uint8_t i)
{

	ShowingFrameBufferIndex=i;
	frameBufferShowing = (disp_y_size + 1) * i;
	//LOG("showing  Line to %d", frameBufferAddress);
	//LOG("showing  Line to %d", frameBufferShowing);
	assertCS();

	LCD_Write_COM(0x37);
	LCD_Write_DATA8(frameBufferShowing >> 8);
	LCD_Write_DATA8(frameBufferShowing & 0x0ff);
	removeCS();
