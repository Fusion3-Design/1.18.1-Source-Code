...
bool FlashStorage::write(uint32_t address, const void *data, uint32_t dataLength)
{
	...
	{
		LOG("Flash write address too low");
		return false;
	}

	if ((uint32_t)FLASH_START + address + dataLength > (uint32_t)&__flash_end__)
	{
		LOG("Flash write address too high");
		return false;
	}

	if ((((uint32_t)FLASH_START + address) & 3) != 0)
	{
		LOG("Flash start address must be on 4-byte boundary\n");
		return false;
	}
	...
	{
		LOG("Failed to unlock flash for write");
	}
	else
	{
		// Write data
