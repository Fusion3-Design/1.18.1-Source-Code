...
const char*  trGrave =			"A\xC0" "E\xC8" "I\xCC"         "O\xD2" "U\xD9" "a\xE0" "e\xE8" "i\xEC" "o\xF2" "u\xF9"        ;
const char*  trAcute =			"A\xC1" "E\xC9" "I\xCD"         "O\xD3" "U\xDA" "a\xE1" "e\xE9" "i\xED" "o\xF3" "u\xFA" "y\xFD";
const char*  trCircumflex =	"A\xC2" "E\xCA" "I\xCE"         "O\xD4" "U\xDB" "a\xE2" "e\xEA" "i\xEE" "o\xF4" "u\xFB"        ;
const char*  trTilde =			"A\xC3"                 "N\xD1" "O\xD5"         "a\xE3"                 "o\xF5"                ;
const char*  trUmlaut =		"A\xC4" "E\xCB" "I\xCF"         "O\xD6" "U\xDC" "a\xE4" "e\xEB" "i\xEF" "o\xF6" "u\xFC" "y\xFF";
const char*  trCircle =		"A\xC5"                                         "a\xE5"                                        ;
const char*  trCedilla =		"C\xC7" "c\xE7";
...
void RawSendStr(const char * str)
{
	while (*str)
	{
		RawSendChar(*str++);
...
String<50> fieldId;
String<2048> fieldVal;	// long enough for about 3 lines of message
...
{
	Machine::getInstance().ProcessReceivedValue(fieldId.c_str(), fieldVal.c_str(), arrayIndices);
	fieldVal.clear();
}

static void EndArray()
{
	Machine::getInstance().ProcessArrayEnd(fieldId.c_str(), arrayIndices);
		...
		else
		{
			switch(state)
			{
				case jsBegin:			// initial state, expecting '{'
					if (c == '{')
					{
						Machine::getInstance().StartReceivedMessage();
						state = jsExpectId;
						fieldVal.clear();
						fieldId.clear();
						arrayDepth = 0;
				...
				case jsExpectId:		// expecting a quoted ID
					switch (c)
					{
						...
						case '}':			// empty object, or extra comma at end of field list
							...
							if (fieldId.size() == 0)
							{
								Machine::getInstance().EndReceivedMessage();
								state = jsBegin;
							}
							else
							{
