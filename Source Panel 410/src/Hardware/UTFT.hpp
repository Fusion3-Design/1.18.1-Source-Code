...
public:
	...
	void fillRect(int x1, int y1, int x2, int y2, Colour grad = 0, uint8_t gradChange = 1, bool vertical=false);
	void fillRoundRect(int x1, int y1, int x2, int y2, Colour grad = 0, uint8_t gradChange = 1);
	void drawCircle(int x, int y, int radius);
	void fillCircle(int x, int y, int radius);
		

	Colour applyColorScale(uint8_t percent, Colour c);
	...
	void drawBinaryBitMap(int x, int y, int sx, int sy, const uint8_t * data);
	...
	static uint16_t GetFontHeight(const uint8_t *f) { return reinterpret_cast<const FontDescriptor*>(f)->y_size; }
		...
		bool SetWriteFrameBuffer(uint8_t i);
		bool ShowFrameBuffer(uint8_t i);
		uint32_t getWriteFrameBuffer()
		{
			return WriteFrameBufferIndex;
		}
		uint32_t getShowingFrameBuffer()
		{
			return ShowingFrameBufferIndex;
		}
...
private:
	...
	DisplayType displayModel;
		uint32_t frameBufferAddress = 0; //frame buffer for writting
		uint32_t frameBufferShowing = 0;
	uint8_t WriteFrameBufferIndex=0;
	uint8_t ShowingFrameBufferIndex=0;
