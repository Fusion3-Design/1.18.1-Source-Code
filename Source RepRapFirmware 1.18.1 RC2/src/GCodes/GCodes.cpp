...
#ifdef HOMED_BEFORE_JOGGED // packetmax
	// packetmax: G1, sans 'E' and 'F6000' present, if a move along any axis is requested and that axis is not homed, return false.
	if (!gb.Seen(extrudeLetter) && gb.Seen(feedrateLetter))
	{
		float specialOffset = gb.GetFValue();
		if (specialOffset == 6000.0) {
			if ((gb.Seen(axisLetters[X_AXIS]) && !GetAxisIsHomed(X_AXIS)) ||
					(gb.Seen(axisLetters[Y_AXIS]) && !GetAxisIsHomed(Y_AXIS)) ||
					(gb.Seen(axisLetters[Z_AXIS]) && !GetAxisIsHomed(Z_AXIS)))
			{
				return 1;
			}
		}
...
// packetmax: Run the file corresponding to HEATER_FAULT_G
const char* HEATER_FAULT_G = "heatfault.g";
void GCodes::HeaterFault()
{
	GCodeBuffer& gb = *(fileGCode);
	DoFileMacro(gb, HEATER_FAULT_G, false);
}

void GCodes::HeaterFaultPause()
{
	GCodeBuffer& gb = *(fileGCode);
	if (LockMovement(gb)) {
		DoPause(gb);
